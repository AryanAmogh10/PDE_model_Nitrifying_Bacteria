...
REGIME, EPS, LAM = info["regime"], info["eps"], info["lambda"]
...
def render_grid(field_getter, rows, row_label, row_cmap, out_name, suptitle, per_row_scale=False):
    ...
    fig.suptitle(f"{suptitle} -- coupled (eps={EPS:g}, Lambda={LAM:g}), {REGIME} regime" + ("" if per_row_scale else f", peak={global_vmax:.3g}"), fontsize=13)
...
render_grid(get_u, SPECIES, SPECIES_LABEL, SPECIES_CMAP,
            f"coupled_commensalism_{REGIME}_bacteria.png", "Bacterial densities (commensalism, rough IC)")
render_grid(get_c, SUBSTRATES, SUB_LABEL, SUB_CMAP,
            f"coupled_commensalism_{REGIME}_substrates.png", "Substrate concentrations", per_row_scale=True)

bmh = np.load(RESULTS_DIR / "boundary_mass_history.npy")
first_touch = bmh[bmh[:, 1] > 1e-8]
if len(first_touch):
    print(f"{REGIME}: biomass first reaches the domain boundary at t = {first_touch[0,0]:.1f}")
else:
    print(f"{REGIME}: biomass never reached the domain boundary within t = {bmh[-1,0]:.0f}")
...
ax.set_ylabel(r"$\int \epsilon |c_t| \, / \, \int |\nabla^2 c|$")
ax.set_title(f"Transient vs diffusion term, eps={EPS:g}, {REGIME} (<<1: QSSA valid, ~1: comparable)")
ax.legend()
out = OUT_DIR / f"coupled_commensalism_{REGIME}_term_ratio.png"
