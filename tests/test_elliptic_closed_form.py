...
from nitrifiers.one_dimensional.elliptic import (
    Grid, solve_newton, solve_picard, normalize_bc_specs, build_laplacian,
    apply_bc, _residual, _assemble_global, SUBSTRATES,
)
from nitrifiers.one_dimensional.relaxation import solve_relaxation
...
def _robin_closed_form(r, kappa, Bi, c_inf, p):
    """Linear reaction-diffusion 0 = Lap(c) - kappa^2 c with robin at r=1,
    -dc/dr = Bi*(c - c_inf), symmetry at r=0. Same solution families as the
    dirichlet/neumann cases, with the amplitude A fixed by the robin condition."""
    k = kappa
    if p == 0:
        A = Bi * c_inf / (k * np.sinh(k) + Bi * np.cosh(k))
        return A * np.cosh(k * r)
    if p == 1:
        A = Bi * c_inf / (k * iv(1, k) + Bi * iv(0, k))
        return A * iv(0, k * r)
    A = Bi * c_inf / (k * np.cosh(k) - np.sinh(k) + Bi * np.sinh(k))
    out = np.empty_like(r)
    mask = r > 1e-12
    out[mask] = A * np.sinh(k * r[mask]) / r[mask]
    out[~mask] = A * k
    return out


def test_robin_matches_closed_form_and_degenerates_correctly():
    """Robin BC vs closed form (all 3 geometries), BC residual from the
    converged profile, and the Bi=0/Bi->inf limits. See README for why."""
    kappa, Bi, c_inf = 1.5, 2.0, 1.0
    K_NH4, u0 = 1.0e7, 0.05

    def robin_specs(B):
        return {"NH4": ("robin", (B, c_inf)), "O2": ("dirichlet", 1.0),
    ...
    for geometry, p in (("slab", 0), ("radial", 1), ("radial", 2)):
        ...
        for N in (100, 400):
            ...
            coeffs = _single_species_coeffs(kappa, K_NH4, u0, c_inf)
            U = {"AOB": np.full(Npts, u0), "NOB": np.zeros(Npts), "CMX": np.zeros(Npts)}
            C, hist, method = solve_newton(coeffs, U, grid, bc_specs=robin_specs(Bi),
                                            maxiter=200, tol=1e-9, relaxation_fallback=False)
            assert hist[-1] < 1e-9, (geometry, p, N, method, hist[-1])
            errs[N] = np.max(np.abs(C["NH4"] - _robin_closed_form(grid.r, kappa, Bi, c_inf, p)))
            # (2): robin condition, recomputed from the converged profile
            minus_dcdr = -(C["NH4"][-1] - C["NH4"][-2]) / grid.h
            assert abs(minus_dcdr - Bi * (C["NH4"][-1] - c_inf)) < 1e-10, (geometry, p, N)
        assert errs[400] < 1e-3, (geometry, p, errs)
        assert errs[400] < errs[100], (geometry, p, errs)

    # (3) limits, spherical
    grid = Grid(N=200, geometry="radial", p=2)
    Npts = grid.N + 1
    coeffs = _single_species_coeffs(kappa, K_NH4, u0, c_inf)
    U = {"AOB": np.full(Npts, u0), "NOB": np.zeros(Npts), "CMX": np.zeros(Npts)}
    base = {"O2": ("dirichlet", 1.0), "NO2": ("dirichlet", 0.0), "NO3": ("dirichlet", 0.0)}
    kw = dict(maxiter=200, tol=1e-9, relaxation_fallback=False)

    C_r0, _, _ = solve_newton(coeffs, U, grid, bc_specs={**base, "NH4": ("robin", (0.0, c_inf))}, **kw)
    C_n0, _, _ = solve_newton(coeffs, U, grid, bc_specs={**base, "NH4": ("neumann", 0.0)}, **kw)
    assert np.array_equal(C_r0["NH4"], C_n0["NH4"])   # bit-identical, not merely close

    C_d, _, _ = solve_newton(coeffs, U, grid, bc_specs={**base, "NH4": ("dirichlet", c_inf)}, **kw)
    gaps = []
    for B in (1e2, 1e4):
        C_r, _, _ = solve_newton(coeffs, U, grid, bc_specs={**base, "NH4": ("robin", (B, c_inf))}, **kw)
        gaps.append(np.max(np.abs(C_r["NH4"] - C_d["NH4"])))
    assert gaps[1] < gaps[0], gaps
    ratio = gaps[0] / gaps[1]          # Bi went up 100x -> gap should shrink ~100x
    assert 50 < ratio < 200, (gaps, ratio)

    # Picard cross-check, spherical
    C_p, hist_p = solve_picard(coeffs, U, grid, bc_specs=robin_specs(Bi), maxiter=5000, tol=1e-9)
    err_p = np.max(np.abs(C_p["NH4"] - _robin_closed_form(grid.r, kappa, Bi, c_inf, 2)))
...
def test_eloi_stiffness_coupled_neumann_is_near_singular():
    """Companion to test_coupled_multi_substrate_neumann_converges above, at
    the OTHER end of the stiffness range: the actual eloi preset, not the
    moderate synthetic Lambda~O(1-10) coefficients used there.

    This case is EXPECTED to fail to converge -- that failure is the thing
    under test, not an accident. The moderate-stiffness test already shows
    the bc_specs/plausibility-guard/degeneracy-fallback machinery all work
    correctly together; this one confirms that eloi's realistic stiffness
    hits a genuine numerical-conditioning wall (a near-singular Jacobian,
    condition number >> 1e14) rather than some other, unrelated failure mode
    (a crash, a non-finite result, or silent wrong convergence).

    Two independent checks, both on the actual eloi coefficients:
      (a) plain Newton (relaxation_fallback=False, so no 5000-step PTC detour
          padding out the test) stalls well above tol within a small
          iteration budget.
      (b) solve_relaxation directly, given a capped step budget, also fails
          to reach tol -- confirming this isn't a "Newton specifically is
          bad at this, PTC would fix it" situation.
    Both stall states are also checked for a near-singular Jacobian, so the
    same underlying conditioning problem is confirmed twice, not assumed
    from (a) alone.
    """
    ...
    U = {"AOB": np.full(Npts, 1.0), "NOB": np.zeros(Npts), "CMX": np.zeros(Npts)}
    tol = 1e-9
    cond_floor = 1e14  # clearly distinguishes near-singular from well-conditioned

    def _jacobian_condition_number(C):
        bc_specs_n = normalize_bc_specs(coeffs, "dirichlet", bc_specs)
        Lap0 = build_laplacian(grid)
        Lap_bc = {}
        for sub in SUBSTRATES:
            bt, val = bc_specs_n[sub]
            Lb, _ = apply_bc(Lap0, np.zeros(Npts), grid, bt, val)
            Lap_bc[sub] = Lb
        F, R, dR = _residual(C, U, Lap_bc, coeffs, bc_specs_n)
        _, J = _assemble_global(Lap_bc, R, dR, C)
        return np.linalg.cond(J.toarray())

    # (a) plain Newton, no relaxation detour -- stalls fast
    C_newton, hist_newton, method = solve_newton(
        coeffs, U, grid, bc_specs=bc_specs, maxiter=50, tol=tol,
        relaxation_fallback=False)
    assert hist_newton[-1] > 1e-3, (method, hist_newton[-1])  # nowhere near tol
    cond_newton = _jacobian_condition_number(C_newton)
    assert cond_newton > cond_floor, cond_newton

    # (b) solve_relaxation directly, capped step budget -- also stalls
    C_relax, hist_relax = solve_relaxation(
        coeffs, U, grid, bc_specs=bc_specs, steady_tol=tol, max_steps=300)
    assert hist_relax[-1] > 1e-3, hist_relax[-1]
    cond_relax = _jacobian_condition_number(C_relax)
    assert cond_relax > cond_floor, cond_relax
...
if __name__ == "__main__":
    ...
    test_robin_matches_closed_form_and_degenerates_correctly()
    test_coupled_multi_substrate_neumann_converges()
    test_eloi_stiffness_coupled_neumann_is_near_singular()
