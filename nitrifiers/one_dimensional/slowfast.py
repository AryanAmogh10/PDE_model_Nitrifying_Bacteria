"""Stage 6: the full slow-scale loop. Each step: solve for c given frozen u
(Newton, falling back to relaxation), then advance u by one dt_slow step."""
...
def solve_c_given_u(coeffs: dict, U: dict, grid: Grid, bc_type: str = "dirichlet",
    ...
    """Newton first (which has its own inner relaxation fallback for a
    detected degenerate Jacobian); this function's own outer fallback only
    fires if Newton returns above tolerance without ever hitting that."""
